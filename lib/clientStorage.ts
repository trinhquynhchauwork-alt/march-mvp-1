...
import type { Profile, InsightResult, MatchedSchool } from "@/types/domain";
...
export function saveSchools(profile: Profile, schools: MatchedSchool[]) {
...
export function loadSchoolsIfMatches(profile: Profile): MatchedSchool[] | null {
  const cachedProfileRaw = sessionStorage.getItem(SCHOOLS_FOR_PROFILE_KEY);
  if (cachedProfileRaw !== JSON.stringify(profile)) return null;
  const raw = sessionStorage.getItem(SCHOOLS_KEY);
  return raw ? (JSON.parse(raw) as MatchedSchool[]) : null;
}
